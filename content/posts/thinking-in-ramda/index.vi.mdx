---
title: 'Tư duy trong Ramda'
date: '2018-01-20'
slug: /vi/posts/thinking-in-ramda
tags:
  - ramda
...
Tôi đã nói về thư viện [Ramda](http://ramdajs.com/) áp dụng cho JavaScript một vài lần:
...
Tôi thấy Ramda là một thư viện được thiết kế độc đáo, cung cấp rất nhiều công cụ cho lập trình hàm trong JavaScript một cách gọn gàng, dễ hiểu.
...
- Tham khảo chúng như hằng số và biến
...
Ý tưởng cơ bản là, nếu bạn gọi một hàm với các đầu vào giống nhau nhiều lần, bạn luôn nhận được một kết quả giống nhau.

Bạn chắc chắn có thể làm một số thứ với các hàm không thuần khiết (và bạn cần phải làm như vậy nếu chương trình của bạn muốn làm bất cứ điều gì thú vị), nhưng phần lớn trường hợp bạn sẽ muốn giữ cho hầu hết các hàm của bạn trở nên thuần khiết.

### Tính bất biến (immutability)

Một khái niệm quan trọng khác trong lập trình hàm là "tính bất biến". Điều đó có nghĩa là gì? "Bất biến" (immutable) có nghĩa là "không thể thay đổi được".
...
Thay vào đó, nếu tôi cần phải thay đổi một cái gì đó trong một mảng hoặc đối tượng, tôi sẽ trả lại một bản sao mới của nó với giá trị đã được thay đổi. Các bài viết sau chúng ta sẽ nói về điều này một cách chi tiết.
...
Martin Fowler có một vài bài viết tuyệt vời về "Tập hợp đường ống", [cách sử dụng các hàm này](http://martinfowler.com/articles/collection-pipeline/) và [cách tái cấu trúc code hiện có vào các tập hợp đường ống](http://martinfowler.com/articles/refactoring-pipelines.html).

Lưu ý rằng tất cả các hàm này (ngoại trừ `reject`) đều có sẵn trên `Array.prototype`, vì vậy bạn không cần Ramda để bắt đầu sử dụng chúng. Tuy nhiên, tôi sẽ sử dụng phiên bản của Ramda cho nhất quán với phần còn lại.
...
Điều này là phổ biến trong lập trình hàm, và chúng ta sẽ nói về nó nhiều hơn nữa trong bài tiếp theo. Chúng ta cũng sẽ nói về việc làm thế nào để kết hợp các hàm có nhiều hơn một tham số.

## 3. Áp dụng từng phần

Trong phần trước, chúng ta đã xem xét các hàm đường ống đơn giản chỉ có một tham số. Vậy nếu chúng ta muốn sử dụng các hàm có nhiều hơn một tham số thì như thế nào?
...
Ramda giải cứu chúng ta một lần nữa với hàm `evolve` của nó. Tôi giới thiệu `evolve` trong một [bài viết trước](http://randycoulman.com/blog/2016/02/16/using-ramda-with-redux/).
...
- Randy Coulman. "Thinking in Ramda", 2016. https://randycoulman.com/blog/categories/thinking-in-ramda/.
- Ramda Documentation. https://ramdajs.com/.
- "Ramda Wiki" GitHub, https://github.com/ramda/ramda/wiki.
