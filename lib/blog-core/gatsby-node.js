...
const initCustomData = require('./custom-data')
...
exports.createSchemaCustomization = ({ actions, schema }, themeOptions) => {
  ...
  createTypes(`
    ...
    interface Post @nodeInterface {
      ...
      fields: MdxFields @mdxpassthrough(fieldName: "fields")
      body: String!
      html: String
      display: Display
      timeToRead: Int
    ...
    dataTypes: [
      {
        ...
        queryChain: (x) => x('experiences'),
      },
      {
        typeName: 'Materials',
        queryChain: (x) => x.select('*').from('learning_materials'),
      },
      {
        typeName: 'Skills',
        queryChain: (x) => x.select('*').from('skills'),
      },
      {
        typeName: 'SkillCategories',
        queryChain: (x) => x.select('*').from('skill_category'),
      },
    ],
  })
}
...
exports.onCreateNode = (
  ...
  if (node.internal.type === `Mdx` && source === postsPath) {
    ...
    const fieldData = {
      ...
      display: node.frontmatter.display || 'SHOW',
      tags: modifiedTags,
